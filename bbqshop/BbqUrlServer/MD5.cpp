...
inline void MD5::ByteToDWord(uint4* Output, unsigned char* Input, uint4 nLength)
{  
	//entry invariants  
	//initialisations  
...
inline void MD5::DWordToByte(unsigned char* Output, uint4* Input, uint4 nLength)
{  
	//entry invariants  
	//transfer the data by shifting and copying  
	unsigned int i = 0;  
...
{  
	//Save number of bits  
	unsigned char Bits[8];  
	DWordToByte(Bits, count, 8);

	//Pad out to 56 mod 64.  
	uint4 nIndex = (uint4)((count[0] >> 3) & 0x3f);  
	uint4 nPadLen = (nIndex < 56) ? (56 - nIndex) : (120 - nIndex);  
	...
	DWordToByte( lpszMD5, state, nMD5Size );  
